...
/** Transparent face artwork shared by the SVG and projected 3D renderers. */
export function renderFaceSvg(d: CharacterDocument, expression?: string) {
  const f = resolveExpression(d, expression),
    c = d.colors;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="768" height="768" viewBox="-130 -130 260 260"><g transform="translate(${f.x} ${f.y - 7}) rotate(${f.tilt})">${f.blush ? `<ellipse cx="-47" cy="24" rx="14" ry="7" fill="${c.accent}" opacity=".65"/><ellipse cx="47" cy="24" rx="14" ry="7" fill="${c.accent}" opacity=".65"/>` : ""}<g transform="translate(0 ${f.eyeY})" fill="${c.eyes}" color="${c.eyes}">${[-1, 1].map((side) => `<g transform="translate(${(side * f.spacing) / 2} 0) rotate(${f.eyeRotation}) scale(${f.eyeScale})">${eye(f.eyes, side).replaceAll("currentColor", c.eyes)}</g>`).join("")}</g><g transform="translate(0 ${f.mouthY})" fill="${c.mouth}" color="${c.mouth}">${mouth(f.mouth).replaceAll("currentColor", c.mouth)}</g></g></svg>`;
}
